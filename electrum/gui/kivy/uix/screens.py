...
from electrum.util import (parse_URI, InvalidWaifuURI, TxMinedInfo, maybe_extract_bolt11_invoice,
...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Waifu address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitcoin.is_address(address):
                        self.app.show_error(_('Invalid Waifu Address') + ':\n' + address)
                        return
