...
class WaifuAverage(ExchangeBase):
...
class WaifuVenezuela(ExchangeBase):
...
#                     for ccy, d in json['rates'].items()])
        json = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/bitnet-io/market_chart?vs_currency=%s&days=max')

        return dict([(datetime.utcfromtimestamp(h[0]/1000).strftime('%Y-%m-%d'), h[1])
                     for ccy, h in json['prices'].items()])
...
class LocalWaifus(ExchangeBase):
...
class MercadoWaifu(ExchangeBase):
