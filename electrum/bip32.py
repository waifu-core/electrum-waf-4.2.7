...
from .util import bfh, bh2u, WaifuException
...
def _CKD_priv(parent_privkey: bytes, parent_chaincode: bytes,
    ...
    try:
        ...
        raise WaifuException('Impossible xprv (not within curve order)') from e
...
class InvalidMasterKeyVersionBytes(WaifuException): pass
...
class BIP32Node(NamedTuple):
    ...
    def from_xkey(cls, xkey: str, *, net=None) -> 'BIP32Node':
        ...
        if len(xkey) != 78:
            raise WaifuException('Invalid length for extended key: {}'
                                   .format(len(xkey)))
        depth = xkey[4]
    ...
    def from_rootseed(cls, seed: bytes, *, xtype: str) -> 'BIP32Node':
        I = hmac_oneshot(b"Waifu seed", seed, hashlib.sha512)
        master_k = I[0:32]
        master_c = I[32:]
        return BIP32Node(xtype=xtype,
